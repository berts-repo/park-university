    ...
    def average_silhouette_score(self):
        ...
        # TODO: determine this average here and assign it to 'a'
            ...
            for other_eg in self.dataset.train:
                # Verify that other_eg is part of the same cluster as eg
                if self.class_of_eg(other_eg) == c and other_eg != eg:
                    distance_sum += self.distance(c, other_eg)
            # Compute average
            a = distance_sum / (self.class_counts[c] - 1)
        ...
        # TODO: determine this average here and assign it to 'b'
            #Initialize to infinity (max size)
            average_distance = float('inf')
            for other_c in range(self.num_classes):
                # Skip the cluster if it is the same as that of eg
                if other_c != c:
                    distance_sum = 0
                    for other_eg in self.dataset.train:
                        # Add up distances from eg to all examples in other_c
                        if self.class_of_eg(other_eg) == other_c:
                            distance_sum += self.distance(other_c, other_eg)

                    other_average = distance_sum / self.class_counts[other_c]
                    average_distance = min(average_distance, other_average)
...
data = Data_from_file('3-clust.csv', prob_test=0, target_index=-1)
kml = K_means_learner(data, 2)
print('initial error: ', kml.average_training_error())

while not kml.learn(1):
    print('current error: ', kml.average_training_error())
print('k=2 final error: ', kml.average_training_error())
kml.show_classes()
...
# ANSWER: Both the sillouette score and elbow methods compare k's cluster to other clusters, that measures the distance from k.
