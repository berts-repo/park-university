...
def create_sudoku_constraints(variables): 
  ...
  # Rule 1: Every variable in a row must have unique values
  constraints += [Constraint([variables[row, col1], variables[row, col2]], ne)
    for row in range(1, 10)
    for col1 in range(1, 10)
    for col2 in range(col1 + 1, 10)]

  # Rule 2: Every variable in a column must have unique values
  constraints += [Constraint([variables[row1, col], variables[row2, col]], ne)
    for col in range(1, 10)
    for row1 in range(1, 10)
    for row2 in range(row1 + 1, 10)]

  # Rule 3: Every variable in a 3x3 block must have unique values
  constraints += [Constraint([variables[r1, c1], variables[r2, c2]], ne)
    for r1 in range(1, 10, 3)
    for c1 in range(1, 10, 3)
