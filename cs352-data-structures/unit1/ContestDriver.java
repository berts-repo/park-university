	    ...
	    for (int round = 1; round <= numRounds; round++) {
		    ...
		    for (int i = 0; i < contestants.size(); i++) {
			    ...
			    if (isAnswerCorrect) {
				    ...
				    contestants.add(0, contestant); // Move to the front
			    } else {
				    contestants.remove(index);
				    contestants.add(contestant); // Move to the end
				    index--;
			    }
			    index++;
		    ...
		    for (Contestant contestant : contestants) {
			    ...
			    System.out.println("contestant: " + contestant + " Total position: " + contestant.getTotalPosition());
			    eachRound++;
		    }
	    }
