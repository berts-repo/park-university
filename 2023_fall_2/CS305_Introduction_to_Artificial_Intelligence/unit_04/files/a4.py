...
# ANSWER: When calculating the mode with such a small dataset of 3 columns besides the key-map, which doesn't represent the distribution.
    bl_acc /= trials
...
# ANSWER: The best method in this sample would be the k-nn, which is determined from the accuracy of the test results. The results were ~2-4% more accurate then the DT reults, and ~70% higher than the baseline. Also, because the test dataset is an instance-based approach, and it was small, it doesn't cost to much. The k-nn algorithm "memorizes" the entire dataset and stores the data as tuples.
